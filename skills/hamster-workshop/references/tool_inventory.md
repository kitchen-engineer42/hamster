...
## Current platform tools (as of John v0.1.20)

### `local_clients/llm/` — workerLLM client

A FastAPI server wrapping SiliconFlow + DeepSeek (today; the URL contract is provider-agnostic so the tech team can swap in production servers later).
...
- **Substitutability** — the URL-env-var contract means the tech team can swap implementations (e.g., on-prem PPX server, different LLM provider) without any template changing.
...
The tool list above is pinned to John v0.1.20. The platform may grow. To re-check:
