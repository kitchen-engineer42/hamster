...
1. **The bundled architecture summary** (`references/john_architecture.md`) — your always-have-in-mind baseline. Read it whenever you need to recall the layout, the 19 core skills, the hooks contract, the templates diff format. Pinned to John v0.2.3. If your template adds fan-out agents, conform to the **agent event contract** section there — copy the event shapes from John core's `agents/knowledge-extractor.md` rather than inventing your own.
