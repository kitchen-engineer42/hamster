...
If you haven't read `references/john_architecture.md` yet, do that first. It's a version-pinned summary of joharnessburg layout, the 26 skills, hooks contract, templates mechanics — tighter than reading the live repo cold.
...
Two access patterns, used together:

1. **The bundled architecture summary** (`references/john_architecture.md`) — your always-have-in-mind baseline. Read it whenever you need to recall the layout, the 26 skills, the hooks contract, the templates diff format. Pinned to John v0.1.12.

2. **Live reads of `$JOHARNESSBURG_PATH`** — for skill bodies, script implementations, current hook behavior. Spawn subagents for deep reads. Don't load whole skill files into your main context unless you're about to override them.
